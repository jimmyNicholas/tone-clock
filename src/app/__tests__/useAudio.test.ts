...
describe('useAudio Hook', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(Oscillator).mockImplementation(() => mockOscillator as unknown as Oscillator)
    vi.mocked(getOsc).mockReturnValue(mockOscillator as unknown as Oscillator)
    vi.mocked(getChordOscillators).mockReturnValue([mockOscillator, mockOscillator, mockOscillator] as unknown as Oscillator[])
    vi.mocked(getClockFrequency).mockReturnValue(440)
    vi.mocked(getClockTriad).mockReturnValue({
      currentTriad: [261.63, 329.63, 392.00],
      nextTriad: [293.66, 369.99, 440.00]
    })
  })
  ...
  describe('Basic Functionality', () => {
    ...
    it('should provide control functions', () => {
      ...
      expect(typeof result.current.toggleChordMode).toBe('function')
  ...
  describe('Chord Mode - The Feature Users Toggle', () => {
    it('should toggle chord mode', () => {
      const { result } = renderHook(() => useAudio(mockTime, true))
